...
router.post('/new', (req, res, next) => {
    ...
    db.query("SELECT user_id FROM ROLE WHERE EMAIL=$1",[activeUserMail], (err,res1) => {
        if(err) {
            ...
            db.query('INSERT INTO REQUESTS(product_id, requester_id, approver_id, requester_comment) VALUES($1,$2,$3,$4) RETURNING *', [productId, res1.rows[0].user_id, approverId, requesterNote], (err,res2) => {
                if(err) {
                    res.statusMessage = err.message;
                    console.log("Status Code: 500"); //Internal Server Error
                    res.status(500).json({"statusText" : err.message});
