...
/**
 * Note : 
 * Primary Key Must Be (SERIAL) autogenerated
 * Products, Role, Request contain a autogenerated serial primary key
 * -- CREATE TABLE REQUEST(REQUEST_ID SERIAL PRIMARY KEY,PRODUCT_ID VARCHAR(5),REQUESTER_ID VARCHAR(5),APPROVER_ID VARCHAR(5) ,REQUESTER_COMMENT VARCHAR(500),APPROVER_COMMENT VARCHAR(500),FOREIGN KEY(PRODUCT_ID) REFERENCES PRODUCTS(PROD_ID),FOREIGN KEY(REQUESTER_ID) REFERENCES ROLE(USER_ID),FOREIGN KEY(APPROVER_ID) REFERENCES ROLE(USER_ID))
 * -- CREATE TABLE REQUEST_STATUS(REQUEST_ID INTEGER,STATUS VARCHAR(100), FOREIGN KEY(REQUEST_ID) REFERENCES REQUEST(REQUEST_ID))
 */
